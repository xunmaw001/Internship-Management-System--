        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '实习日志标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.shixirizhiName" placeholder="实习日志标题" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shixirizhi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/shixishengguanli/upload/shixirizhiMuBan.xls"
                        >批量导入实习日志数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shixirizhi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="shixirizhiUploadSuccess"
                                :on-error="shixirizhiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shixirizhi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入实习日志数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shixirizhi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shixirizhi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixirizhiUuidNumber"
                                   header-align="center"
                                   label="实习日志编号">
                        <template slot-scope="scope">
                            {{scope.row.shixirizhiUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('shixirizhi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shixirizhi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shixirizhi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                xueshengId : null,
                shixirizhiUuidNumber : null,
                shixirizhiName : null,
                riqiTime : null,
                shixirizhiContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '学生邮箱': 'xueshengEmail',
                //本表字段
                     '实习日志编号': "shixirizhiUuidNumber",
                     '实习日志标题': "shixirizhiName",
                     '日期': "riqiTime",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shixirizhi',//当前表表名,
                        sumColum : 'shixirizhi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shixirizhi_name',//分组字符串字段
                        // types : 'shixirizhi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.shixirizhiName!= '' && this.searchForm.shixirizhiName!= undefined) {
                    params['shixirizhiName'] = '%' + this.searchForm.shixirizhiName + '%'
                }
                                
                params['shixirizhiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            shixirizhiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shixirizhi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "shixirizhi/update",
                        method: "post",
                        data: {
                            id:id,
//                            shixirizhiTypes:shixirizhiTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
