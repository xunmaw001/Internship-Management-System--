	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhaopinToudiId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('zhaopinToudi', this.zhaopinToudiId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.zhaopinToudiPhoto ? this.detail.zhaopinToudiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhaopinToudiYuyue', {
									xueshengId: _this.user.id,
									zhaopinToudiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/zhaopinToudiYuyue/list');
							}
						}
					});
				},
