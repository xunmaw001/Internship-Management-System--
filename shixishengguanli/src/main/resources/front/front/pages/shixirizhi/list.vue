	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.shixirizhiName" type="text" placeholder="实习日志标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('shixirizhi','新增')" @click="onAddTap()"
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.shixirizhiName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('shixirizhi','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <!-- 当前表 -->
                 <view v-show="item.shixirizhiUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">实习日志编号</text>
                     <text style="width: 70%">
                         <text v-if="item.shixirizhiUuidNumber != null">{{item.shixirizhiUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.riqiTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">日期</text>
                    <text style="width: 70%">
                        <text v-if="item.riqiTime != null">{{item.riqiTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
                <view v-show="item.shixirizhiContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">实习日志内容</text>
                    <view style="width: 70%" v-html="item.shixirizhiContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('shixirizhi','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('shixirizhi','审核') && item.shixirizhiYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
		...
		methods: {
			...
			async upCallback(mescroll) {
				let params = {
					page: mescroll.num,
					limit: mescroll.size,
					xueshengId:this.userId
				}

                let _this = this
                let res = await this.$api.page(`shixirizhi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shixirizhi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.shixirizhiShenheTime=shijian;
                            await _this.$api.update(`shixirizhi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.shixirizhiName) {
                    searchForm['shixirizhiName'] = this.searchForm.shixirizhiName
                }

                let res = await this.$api.page(`shixirizhi`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.shixirizhiContent!=null){
                        item.shixirizhiContent = item.shixirizhiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
