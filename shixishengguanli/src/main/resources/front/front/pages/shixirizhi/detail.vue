	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('shixirizhi', this.shixirizhiId);
                    this.detail = res.data
					this.detail.shixirizhiContent = this.detail.shixirizhiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.shixirizhiPhoto ? this.detail.shixirizhiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shixirizhiYuyue', {
									xueshengId: _this.user.id,
									shixirizhiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/shixirizhiYuyue/list');
							}
						}
					});
				},
