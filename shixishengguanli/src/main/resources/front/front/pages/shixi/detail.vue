	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('shixi', this.shixiId);
                    this.detail = res.data
					this.detail.tonggongsixiaoyouContent = this.detail.tonggongsixiaoyouContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					this.detail.danweizhuanyeContent = this.detail.danweizhuanyeContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					this.detail.danweirenshiContent = this.detail.danweirenshiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					this.detail.shixiContent = this.detail.shixiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.shixiPhoto ? this.detail.shixiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shixiYuyue', {
									xueshengId: _this.user.id,
									shixiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/shixiYuyue/list');
							}
						}
					});
				},
